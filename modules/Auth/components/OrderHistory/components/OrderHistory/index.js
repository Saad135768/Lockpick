...
const OrderHistory = ({ data, ...props }) => {
  ...
  const { data: getOrderData, loading } = useQuery(GET_ORDER, { variables: { _id } })
        ...
        <div className={classes.OrderSummaryInfo}>
          <Grid container spacing={1}>
            ...
            <Grid item lg={12} md={12} sm={12} xs={12}>
             {!!date && <h4 className={classes.OrdePlaced}>
                Placed on { new Date(+date).toLocaleString('en-us', { day: 'numeric', month: 'numeric', year: 'numeric' })}
              </h4>
}
            </Grid>
            <Grid item lg={6} md={6}>
              <div className={classes.OrderItems}>
                <h4>
                  item : <span> {pathOr(0, ['variations', 'length'], data)}</span>
                </h4>
              </div>
            </Grid>
            <Grid item lg={6} md={6}>
              <div className={classes.OrderTotal}>
                <h4>
                  Total : <span> ${pathOr(0, ['totals', 'total'], data).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')} </span>
                </h4>
              </div>
            </Grid>
          </Grid>
